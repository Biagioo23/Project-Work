...
# Chiamata alla funzione di login all'inizio
login()
...
if st.session_state.logged_in:
    ...
    # Caricamento dati per tutta la sessione.
    # Usiamo 'public.nome_tabella' come specificato
    with st.spinner("Caricamento dati dal database..."):
        df_iscrizioni = load_table("public.iscrizioni")
        df_stage = load_table("public.stage")
        df_corso_docenti = load_table("public.corso_docenti")
        df_corsi = load_table('public.corsi')
        df_corso_materie = load_table('public.corso_materie_its')
        df_docenti = load_table('public.docenti')
        df_ore_alunno = load_table('public.ore_alunno')
    ...
    if role == "governance":
        ...
        if 'ore lavoro' in df_corso_docenti.columns and not df_corso_docenti.empty:
            # Assicurati che 'ore lavoro' sia numerico
            df_corso_docenti['ore lavoro'] = pd.to_numeric(df_corso_docenti['ore lavoro'], errors='coerce')
            df_docenti_pulito = df_corso_docenti.dropna(subset=['ore lavoro'])

            if not df_docenti_pulito.empty:
                df_docenti_ore_totali = df_docenti_pulito.groupby(['cognome', 'nome'])['ore lavoro'].sum().reset_index()
                ...
                st.warning("⚠️ Nessun dato numerico valido per le 'ore lavoro' nella tabella 'corso_docenti'.")
        else:
            st.warning("⚠️ La colonna 'ore lavoro' non è presente in 'corso_docenti' o la tabella è vuota.")
